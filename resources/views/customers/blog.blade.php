            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="card card-side bg-base-100 shadow-xl flex-col md:flex-row">
                    ...
                    <div class="card-body md:w-2/3">
                        <div class="text-sm opacity-70">{{ \Carbon\Carbon::parse($item->created_at)->format('d F Y') }}
                        </div>
                        ...
                        <div class="card-actions justify-end">
                            <a href="{{ route('blog.show', $item->id) }}"
                                class="btn btn-link text-[#FF0000] hover:text-[#cc0000]">
                                Read More
                    ...
                    <div class="card-body">
                        <div class="mb-8">
                            ...
                            <form action="{{ route('blog.search') }}" method="GET" class="join w-full">
                                <input type="text" name="query" class="input input-bordered join-item w-full"
                                    placeholder="Search..." />
                        ...
                        <div>
                            ...
                            <div class="space-y-4">
                                ...
                                @php
                                $recentImage = $recent->gambar ? asset('storage/' . $recent->gambar) :
                                asset('images/default.jpg');
                                @endphp
                                <div class="flex gap-3">
                                    <div class="w-20 h-20 flex-shrink-0">
                                        <img src="{{ $recentImage }}" alt="{{ $recent->judul }}"
                                            class="w-full h-full object-cover rounded-md" />
                                    </div>
                                    <div>
                                        <a href="{{ route('blog.show', $recent->id) }}"
                                            class="font-semibold hover:text-[#FF0000] transition-colors">
                                            {{ Str::limit($recent->judul, 40) }}
                                        </a>
                                        <p class="text-sm opacity-70">
                                            {{ \Carbon\Carbon::parse($recent->created_at)->format('M d, Y') }}</p>
                                    </div>
                                </div>
                                @endforeach
                            </div>
